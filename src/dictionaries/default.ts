...
export const DEFAULT_DETERMINISTIC = [
  // --- 一般表記ゆれ（固有名詞以外の共通ルール） ---
  ...
  // { pattern: /検知する言葉/g, correct: '修正後の言葉', reason: '固有名詞の修正' },
];
...
export const DEFAULT_CONSISTENCY = [
  // =========================================================================
...
【A】 チャンネル基本情報
==========================================================
（※ 以下は書き方の「例」です。新しいチャンネルの情報に書き換えて使ってください）
- チャンネル名：〇〇公式チャンネル（URL: https://www.youtube.com/@example）
- 配信内容：動画編集初心者向けのノウハウやツールの解説コンテンツ。

==========================================================
【B】 チャンネルのスタンス・想定ターゲット（トーン＆マナー）
==========================================================
（※ 以下は書き方の「例」です。新チャンネルの視聴者層やトーンに合わせて書き換えてください）
- ターゲット視聴者：20代〜30代の動画編集を副業にしたい初心者。（※専門用語は極力使わず、かみ砕いて説明する層）
- 語り口・ブランド哲学：親しみやすい「先輩」のようなスタンス。「〜だ・である」といった高圧的なトーンを避け、「〜ですよね！」と共感から入る。
- 提供する価値感：視聴者に「自分にもできそう」という『希望』と、「これならすぐ始められる」という『具体のアクション』を提供する。

==========================================================
【C】 登場人物・関係者
==========================================================
- 〇〇 太郎（まるまる たろう）：当チャンネルのメインパーソナリティ。（※〇〇二郎さんとの混同に注意）
- HIKAKIN（ヒカキン）：日本のトップYouTuber。名前はすべて大文字のアルファベットが正式表記。
- はじめしゃちょー：人気YouTuber。（※「はじめ社長」や「ハジメシャチョー」等の誤変換に注意）
・（その他、プロジェクト固有の人物名等があればここに追加してください）

==========================================================
【D】 企業・ブランド・サービス・専門用語の正式表記
==========================================================
（※ 以下は書き方の「例」です。不要なものは消して書き換えてください）
- YouTube：（※大文字・小文字に注意。youtubeやYou Tube等は不可）
- Instagram：（※インスタグラムやインスタ等の表記ゆれに注意）
- Adobe Premiere Pro：動画編集ソフト。（※プレミアプロ等と略さないこと）
・（その他、プロジェクト固有のサービス名等があればここに追加してください）
