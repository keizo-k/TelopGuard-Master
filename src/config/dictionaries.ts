...
export const STYLE_DICTIONARY = [
  // --- ひらがなで書く ---
  // ※「か月」などは全体ファイルチェック(CONSISTENCY_CHECK_DICTIONARY)に移行したため機械的強制からは除外
...
// ==========================================
// 1.8 ファイル全体での表記ゆれ（一貫性）チェック
// ==========================================
// ここに登録されたグループは、ファイル内で複数の表記が「混在」している場合にのみWarningを出します。
// （例：「1ヶ月」と「1か月」が両方存在する場合に警告・ハイライトする）
export const CONSISTENCY_CHECK_DICTIONARY = [
  {
  ...
  },
  {
    groupName: '「とき／時」の表記',
    variants: [
      // 助詞や用言のあとの「とき」を拾う。名詞の一部（時間・時代・時計など）は除外
      { pattern: /(?<=[あ-んア-ン])とき/g, label: 'ひらがな（とき）' },
