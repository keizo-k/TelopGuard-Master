...
1. **指示事項**:
    ...
    - **【重要】「か月 / ヶ月」「とき / 時」などの漢字・ひらがな・記号の表記ゆれについては、プログラム側でファイル全体の一貫性を自動チェックするため、AIは一切指摘しないでください。直そうとする提案は絶対に行わないこと。**
    - 「何年」「何回」などの助数詞・量を表す言葉や記号・数字についても、別のプログラムで自動修正されるため無視してください。
    - **①②③等の丸付き数字・(1)(2)等の括弧付き数字は、それ自体を指摘しないでください。**

2. **固有名詞・サブカル用語のWeb検索による検証（重要）**:
    - **日本の漫画、アニメ、ネットスラングなどのサブカル用語の可能性が高い用語を検出した場合は、必ずGoogle検索ツールで検証してください。文脈として正しい用語であるか、または表記間違い（タイポ）がないかを確認し、根拠となるURLとともに「正式な表記」をサジェストしてください。**
    - **人名、会社名、サービス名など「固有名詞」と思われるものは、文脈に合っているか、正式表記（大文字小文字の違いなど）に誤りがないかをGoogle検索ツールで確認し、正式な表記を根拠となるURLとともにサジェストしてください。**
    - **検索して確認を行った場合は、理由（reason）の欄に必ず『Web検索で確認済み (URL: https://...)』のように参照元のURLを併記してください。**
