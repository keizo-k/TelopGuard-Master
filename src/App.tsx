...
import { ShortcutAction, ShortcutKeyBinding, DEFAULT_SHORTCUTS, matchesShortcut } from './config/shortcuts';
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="p-3 border-b border-slate-700 bg-[#1e293b] flex justify-between items-center shrink-0">
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        <button onClick={() => setIsSettingsOpen(true)} className="text-[10px] border border-transparent hover:border-slate-600 text-slate-400 hover:text-slate-200 hover:bg-slate-700 px-2 py-1 rounded transition-colors flex items-center gap-1" title="設定 (ショートカットキーなど)">
                            <Settings size={12} />
                            設定
                        </button>
                        {lines.length > 0 && (
                            <button onClick={() => { setLines([]); setLoadedFile(null); setSelectedIndex(-1); }} className="text-xs text-slate-500 hover:text-slate-300">クリア</button>
                        )}
                        {lines.length > 0 && (
                            <button onClick={copyReport} className="text-xs bg-slate-700 hover:bg-slate-600 text-slate-200 px-3 py-1.5 rounded border border-slate-600 transition-colors flex items-center gap-2 group/btn" title="Chatwork用レポートをコピー">
                                ...
                                <kbd className="hidden sm:inline-block text-[9px] font-normal text-slate-400 bg-slate-800 px-1.5 py-0.5 rounded border border-slate-700 font-mono group-hover/btn:text-slate-300">Ctrl+Shift+C</kbd>
                            </button>
                        )}
                        <button
                            onClick={runAICheck}
